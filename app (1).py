...
def save_hist(df):
    df_sorted = df.sort_values(["Semaine", "Séance", "Exercice", "Série"], ignore_index=True)
    data = [df_sorted.fillna("").columns.values.tolist()] + df_sorted.fillna("").values.tolist()
    ws_h.clear()
            ...
            with st.expander(f"🔹 {exo_base.upper()}", expanded=expanded_state):
                ...
                if hist_weeks and st.session_state.settings['show_previous_weeks'] > 0:
                    ...
                    for w_num in weeks_to_show:
                        h_data = f_h[(f_h["Semaine"] == w_num) & (f_h["Poids"] > 0)].sort_values("Série")
                        if not h_data.empty:
                ...
                if not curr.empty and not is_reset and exo_final not in st.session_state.editing_exo:
                    st.markdown("##### ✅ Validé")
                    curr_sorted = curr[["Série", "Reps", "Poids", "Remarque"]].sort_values("Série")
                    st.dataframe(curr_sorted.style.apply(style_comparaison, axis=1, hist_prev=hist_prev_df).format({"Poids": "{:g}"}), hide_index=True, use_container_width=True)
