...
def get_hist():
    try:
        ...
        # Sécurité : Remplacer les muscles vides par "Autre"
    ...
    for idx_j, j in enumerate(jours):
        with st.expander(f"📦 {j}"):
            ...
            for i, ex_obj in enumerate(prog[j]):
                name, nb_sets, muscle = ex_obj["name"], ex_obj["sets"], ex_obj.get("muscle", "Autre")
            ...
            if st.button("➕ Ajouter Exo", key=f"ba_{j}") and ni:
    ...
    if prog:
        ...
        for i, ex_obj in enumerate(prog[choix_s]):
            ...
            with st.expander(f"🔹 {exo_base.upper()}", expanded=True):
                var = st.selectbox("Matériel :", ["Standard", "Barre", "Haltères", "Poulie", "Machine", "Lesté"], key=f"v_{exo_base}_{i}")
                ...
                if len(last_s) > 0:
                    ...
                    df_disp = pd.concat([h_only[h_only["Semaine"] == s] for s in last_s])
                    st.dataframe(df_disp[["Semaine", "Série", "Reps", "Poids", "Remarque"]], hide_index=True, use_container_width=True)
...
with tab3:
    if not df_h.empty:
        ...
        if not activity.empty: st.write(" ".join(["🟩" if datetime.now().strftime("%Y-%m-%d") == d else "⬜" for d in activity.index[-30:]]))
        else: st.info("ℹ️ Enregistre ta première séance pour voir le calendrier !")
