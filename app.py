...
HIST_COLUMNS = ["Semaine", "Séance", "Exercice", "Série", "Reps", "Poids", "Remarque", "Muscle", "Date"]
MUSCLE_OPTIONS = ["Pecs", "Dos", "Jambes", "Épaules", "Bras", "Abdos", "Autre"]
RECUP_MUSCLES = ["Pecs", "Dos", "Jambes", "Épaules", "Bras", "Abdos"]
...
def normalize_history_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Série"] = pd.to_numeric(df["Série"], errors="coerce").fillna(1).astype(int)

    for col in ["Séance", "Exercice", "Remarque", "Muscle", "Date"]:
...
def get_rep_estimations(one_rm: float) -> dict:
    return {
        r: round(one_rm * pct, 1)
        for r, pct in {1: 1.0, 3: 0.94, 5: 0.89, 8: 0.81, 10: 0.75, 12: 0.71}.items()
    }
...
def safe_contains(series: pd.Series, value: str) -> pd.Series:
    return series.fillna("").astype(str).str.contains(value, regex=False, na=False)


def style_comparaison(row: pd.Series, hist_prev: pd.DataFrame) -> list:
    if hist_prev is None or hist_prev.empty:
        return ["", "", "", ""]

    prev_set = hist_prev[hist_prev["Série"] == row["Série"]]
    v = "background-color: rgba(0, 255, 127, 0.2); color: #00FF7F;"
    r = "background-color: rgba(255, 69, 58, 0.2); color: #FF453A;"
    colors = ["", "", "", ""]
...
def get_current_week(df_h: pd.DataFrame) -> int:
    return int(df_h["Semaine"].max() if not df_h.empty else 1)
...
def build_muscle_mapping(programme: dict) -> dict:
    mapping = {}
    for session_name, exercises in programme.items():
        for ex in exercises:
            mapping[ex["name"]] = ex.get("muscle", "Autre")
    return mapping


def enrich_history_with_muscles(df_h: pd.DataFrame, programme: dict) -> pd.DataFrame:
    if df_h.empty:
        return df_h

    muscle_mapping = build_muscle_mapping(programme)
    df_copy = df_h.copy()
    df_copy["Muscle"] = (
        df_copy["Exercice"]
        .apply(get_base_name)
        .map(muscle_mapping)
        .fillna(df_copy["Muscle"])
        .replace("", "Autre")
    )
    return df_copy


def get_session_completion(df_h: pd.DataFrame, programme: dict, semaine: int, seance: str) -> tuple[int, int]:
    if seance not in programme:
        return 0, 0

    seance_data = df_h[(df_h["Séance"] == seance) & (df_h["Semaine"] == semaine)]
    exos_prog = len(programme[seance])

    if seance_data.empty:
        return 0, exos_prog

    exos_done_or_skipped = len(
        seance_data[
            (seance_data["Poids"] > 0)
            | (seance_data["Remarque"].fillna("").str.contains("SKIP", na=False))
        ]["Exercice"].unique()
    )
    return exos_done_or_skipped, exos_prog


def get_next_session(programme: dict, df_h: pd.DataFrame, semaine: int) -> str | None:
    for seance in programme.keys():
        done_count, total_count = get_session_completion(df_h, programme, semaine, seance)
...
def muscle_flappy_game():
    ...
    <script>
        ...
        let biceps = {
            ...
            w: 35,
            h: 35,
            gravity: isMobile ? 0.15 : 0.35,
            velocity: 0,
            lift: isMobile ? -5.5 : -7.5,
            maxVelocity: 8
        };
        ...
        let baseSpeed = isMobile ? 1.2 : 2.5;
        ...
        function draw() {
            ...
            if (gameStarted && !gameOver) {
                ...
                let currentSpeed = baseSpeed + (Math.floor(score / 20) * 0.1);

                let spawnInterval = isMobile ? 220 : 280;
...
def get_gs():
    try:
        ...
        return sh.get_worksheet(0), sh.worksheet("Programme"), None
    except Exception as e:
        return None, None, str(e)


ws_h, ws_p, gs_error = get_gs()
...
def save_hist(df: pd.DataFrame):
    if ws_h is None:
        st.error("Google Sheets indisponible.")
        return False

    try:
        clean_df = normalize_history_df(df)
        payload = [clean_df.columns.values.tolist()] + clean_df.fillna("").values.tolist()
        ws_h.clear()
        ws_h.update(payload, value_input_option="USER_ENTERED")
...
def save_prog(prog_dict: dict):
    if ws_p is None:
        st.error("Google Sheets indisponible.")
        return False

    try:
        ws_p.update_acell("A1", json.dumps(prog_dict, ensure_ascii=False))
        invalidate_data_cache()
        return True
    except Exception as e:
        st.error(f"Erreur lors de la sauvegarde du programme : {e}")
        return False


df_h = get_hist()
prog = get_prog()
df_h = enrich_history_with_muscles(df_h, prog)

if gs_error:
    st.warning(f"Connexion Google Sheets indisponible : {gs_error}")

# Tabs
tab_home, tab_p, tab_s, tab_st, tab_g = st.tabs(
    ["🏠 ACCUEIL", "📅 PROGRAMME", "🏋️‍♂️ MA SÉANCE", "📈 PROGRÈS", "🎮 ARCADE"]
)
...
with tab_home:
    ...
    st.markdown(
        "<h1 style='text-align: center; margin-top: 5px; margin-bottom: 20px;'>💪 MUSCU TRACKER PRO</h1>",
        unsafe_allow_html=True,
    )
    st.markdown("## 🎯 TABLEAU DE BORD")

    s_act = get_current_week(df_h)
    next_session = get_next_session(prog, df_h, s_act)

    week_df = df_h[df_h["Semaine"] == s_act]
    vol_week = int((week_df["Poids"] * week_df["Reps"]).sum())
    vol_week_formatted = f"{vol_week:,}".replace(",", " ")

    sessions_done = len(week_df[week_df["Poids"] > 0]["Séance"].unique())
    ...
    with col_m1:
        st.metric("💪 Exercices", len(week_df[week_df["Poids"] > 0]["Exercice"].unique()))

    with col_m2:
        st.metric("🔢 Séries", len(week_df[week_df["Poids"] > 0]))

    with col_m3:
        total_reps = int(week_df["Reps"].sum())
    ...
    for idx_j, j in enumerate(jours):
        with st.expander(f"📦 {j}"):
            ...
            if c_s1.button("⬆️ Monter Séance", key=f"up_s_{j}") and idx_j > 0:
                ...
                new_prog = {k: prog[k] for k in jours}
                if save_prog(new_prog):
                    st.rerun()
            ...
            for i, ex in enumerate(prog[j]):
                with st.form(key=f"form_exo_{j}_{i}", clear_on_submit=False):
                    c1, c2, c3, c4, c5, c6 = st.columns([3, 1.5, 1.5, 0.7, 0.7, 0.7])

                    ex_name = c1.text_input("Exercice", value=ex["name"], key=f"p_name_{j}_{i}")
                    ex_sets = c2.number_input(
                        "Sets",
                        min_value=1,
                        max_value=15,
                    ...
                    ex_muscle = c3.selectbox(
                        "Muscle",
                        MUSCLE_OPTIONS,
                        index=MUSCLE_OPTIONS.index(ex.get("muscle", "Autre"))
                        if ex.get("muscle", "Autre") in MUSCLE_OPTIONS
                        else MUSCLE_OPTIONS.index("Autre"),
                        key=f"p_muscle_{j}_{i}",
                    )

                    save_btn = st.form_submit_button("💾 Sauver")
                    up_btn = c4.form_submit_button("⬆️")
                    down_btn = c5.form_submit_button("⬇️")
                    del_btn = c6.form_submit_button("🗑️")

                    if save_btn:
                        prog[j][i]["name"] = ex_name
                        prog[j][i]["sets"] = ex_sets
                        prog[j][i]["muscle"] = ex_muscle
                        if save_prog(prog):
                    ...
                    if down_btn and i < len(prog[j]) - 1:
                        prog[j][i], prog[j][i + 1] = prog[j][i + 1], prog[j][i]
                        if save_prog(prog):
                            st.rerun()
            ...
            with st.form(key=f"add_exo_{j}", clear_on_submit=True):
                cx, cm, cs = st.columns([3, 2, 1])
                ni = cx.text_input("Nouvel exo", key=f"ni_{j}")
                nm = cm.selectbox("Groupe", MUSCLE_OPTIONS, key=f"nm_{j}")
                ns = cs.number_input("Séries", 1, 15, 3, key=f"ns_{j}")
                add_exo_submit = st.form_submit_button("➕ Ajouter")

                if add_exo_submit and ni:
                    prog[j].append({"name": ni, "sets": int(ns), "muscle": nm})
                    if save_prog(prog):
                        st.rerun()

    with st.form("create_session_form", clear_on_submit=True):
        nvs = st.text_input("➕ Créer séance")
        create_session_submit = st.form_submit_button("🎯 Valider")
        if create_session_submit and nvs:
            prog[nvs] = []
            if save_prog(prog):
                st.rerun()
...
with tab_s:
    if prog:
        ...
        s_default = get_current_week(df_h)
        s_act = c_h2.number_input("Semaine actuelle", 1, 52, s_default)

        def get_current_session():
            return get_next_session(prog, df_h, s_act)
        ...
        if c_h3.button("🚩 Séance Manquée", use_container_width=True):
            ...
            if save_hist(pd.concat([df_h, m_rec], ignore_index=True)):
                st.rerun()
            ...
            if not trained_this_week.empty:
                ...
                if pd.notna(last_d) and last_d != "":
                    try:
                        ...
                        pass
            html_recup += (
                f"<div class='recup-card'><small>{m.upper()}</small><br>"
                f"<span class='status-dot' style='background-color:{sc}'></span>"
                f"<b style='color:{sc}; font-size:10px;'>{lab}</b></div>"
            )
        st.markdown(html_recup + "</div>", unsafe_allow_html=True)

        curr_session_df = df_h[(df_h["Séance"] == choix_s) & (df_h["Semaine"] == s_act)]
        prev_session_df = df_h[(df_h["Séance"] == choix_s) & (df_h["Semaine"] == s_act - 1)]

        vol_curr = (curr_session_df["Poids"] * curr_session_df["Reps"]).sum()
        vol_prev = (prev_session_df["Poids"] * prev_session_df["Reps"]).sum()

        if vol_prev > 0:
            ratio = min(vol_curr / vol_prev, 1.2)
            st.markdown(
                f"""<div class='vol-container'><small>⚡ Volume : <b>{int(vol_curr)} / {int(vol_prev)} kg</b></small><div class='xp-bar-bg'><div class='vol-bar {"vol-overload" if vol_curr >= vol_prev else ""}' style='width: {min(ratio*100, 100)}%;'></div></div></div>""",
                unsafe_allow_html=True,
            )
        ...
        for i, ex_obj in enumerate(prog[choix_s]):
            exo_base = ex_obj["name"]
            p_sets = int(ex_obj.get("sets", 3))
            muscle_grp = ex_obj.get("muscle", "Autre")

            last_var_data = df_h[
                (safe_contains(df_h["Exercice"], exo_base)) &
                (df_h["Séance"] == choix_s)
            ]
            ...
            exo_completed = (
                not curr_all.empty and (
                    (curr_all["Poids"].sum() > 0 or curr_all["Reps"].sum() > 0)
                    or curr_all["Remarque"].fillna("").str.contains("SKIP", na=False).any()
                )
            )
            ...
            if st.session_state.settings["auto_collapse"]:
                expanded_state = (
                    (not exo_completed)
                    or exo_base in [e.split("(")[0].strip() for e in st.session_state.editing_exo]
                )
            else:
                expanded_state = True
            ...
            with st.expander(f"🔹 {exo_base.upper()}", expanded=expanded_state):
                ...
                is_reset = (
                    not curr.empty
                    and (curr["Poids"].sum() == 0 and curr["Reps"].sum() == 0)
                    and "SKIP" not in str(curr["Remarque"].iloc[0])
                )
                ...
                if not curr.empty and not is_reset and exo_final not in st.session_state.editing_exo:
                    ...
                    st.dataframe(
                        ...
                        .style.apply(style_comparaison, axis=1, hist_prev=hist_prev_df)
                        .format({"Poids": "{:g}"}),
                        hide_index=True,
                        use_container_width=True,
                    )
                    ...
                    if not curr.empty:
                        for _, r in curr.iterrows():
                            if r["Série"] <= p_sets:
                                df_base.loc[df_base["Série"] == r["Série"], ["Reps", "Poids", "Remarque"]] = [
                                    r["Reps"], r["Poids"], r["Remarque"]
                                ]

                    with st.form(key=f"form_save_{exo_final}_{s_act}", clear_on_submit=False):
                        ed = st.data_editor(
                            df_base,
                            num_rows="dynamic",
                            key=editor_key,
                        ...
                        save_submit = c_save.form_submit_button("💾 Enregistrer")
                        skip_submit = c_skip.form_submit_button("⏩ Skip Exo")

                        if save_submit:
                            v = ed.copy()
                            v["Semaine"] = s_act
                            v["Séance"] = choix_s
                            v["Exercice"] = exo_final
                            v["Muscle"] = muscle_grp
        ...
        st.info("Ajoute d'abord une séance dans l'onglet Programme.")

# --- ONGLET PROGRÈS ---
with tab_st:
    if not df_h.empty:
        v_tot = int((df_h["Poids"] * df_h["Reps"]).sum())
        v_tot_formatted = f"{v_tot:,}".replace(",", " ")
        ...
        prev_r = noms[idx - 1] if idx > 0 else "DÉBUT"
        curr_r = noms[idx]
        next_r = noms[idx + 1] if idx < len(noms) - 1 else "MAX"

        next_p = paliers[idx + 1] if idx < len(paliers) - 1 else paliers[-1]
        next_p_formatted = f"{next_p:,}".replace(",", " ")

        xp_ratio = min((v_tot - paliers[idx]) / (next_p - paliers[idx]), 1.0) if next_p > paliers[idx] else 1.0

        st.markdown(
            f"""<div class='rank-ladder'><div class='rank-step completed'><small>PASSÉ</small><br>{prev_r}</div><div style='font-size: 20px; color: #58CCFF;'>➡️</div><div class='rank-step active'><small>ACTUEL</small><br><span style='font-size:18px;'>{curr_r}</span></div><div style='font-size: 20px; color: #58CCFF;'>➡️</div><div class='rank-step'><small>PROCHAIN</small><br>{next_r}</div></div><div class='xp-container'><div class='xp-bar-bg'><div class='xp-bar-fill' style='width:{xp_ratio*100}%;'></div></div><div style='display:flex; justify-content: space-between;'><small style='color:#00FF7F;'>{v_tot_formatted} kg</small><small style='color:#58CCFF;'>Objectif : {next_p_formatted} kg</small></div></div>""",
            unsafe_allow_html=True,
        )
        ...
        for m in labels:
            muscle_data = df_p[df_p["Muscle"] == m]
            m_max = muscle_data["1RM"].max() if not muscle_data.empty else 0
        ...
        if any(s > 0 for s in scores):
            ...
            if valid_scores:
                ...
                msg = f"🛡️ Profil dominé par {top_m}."

            if scores[labels.index("Jambes")] == 0:
                msg += " Pense aux jambes !"
        ...
        if not df_p_filt.empty:
            ...
            for idx, (ex_n, row) in enumerate(podium.iterrows()):
                with p_cols[idx]:
                    st.markdown(
                        f"<div class='podium-card {clss[idx]}'><small>{meds[idx]}</small><br><b>{ex_n}</b><br><span style='color:#58CCFF; font-size:22px;'>{row['1RM']:.1f}kg</span></div>",
                        unsafe_allow_html=True,
                    )
        ...
        st.dataframe(
            df_e[["Semaine", "Série", "Reps", "Poids", "Remarque", "Muscle"]].sort_values("Semaine", ascending=False),
            hide_index=True,
        )
    else:
