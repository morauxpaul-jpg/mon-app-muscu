...
def get_hist():
    try:
        ...
        # FIX : Garantir les colonnes Muscle et Date si absentes du Sheet
...
# --- TAB 2 : MA SÉANCE (FIX DOUBLE AFFICHAGE & CYBER-PR) ---
with tab2:
                    ...
                    if c_v.button(f"💾 Enregistrer", key=f"sv_{exo_final}"):
                        ...
                        if not v.empty:
                            ...
                            if new_1rm > old_1rm and old_1rm > 0:
                                st.balloons(); st.markdown(f"<div class='pr-alert'>🚀 NEW PERSONAL RECORD : {round(new_1rm, 1)}kg !</div>", unsafe_allow_html=True)

                        v["Semaine"], v["Séance"], v["Exercice"] = s_act, choix_s, exo_final
                        v["Muscle"], v["Date"] = muscle_grp, datetime.now().strftime("%Y-%m-%d")
                        mask = (df_h["Semaine"] == s_act) & (df_h["Exercice"] == exo_final) & (df_h["Séance"] == choix_s)
                        save_hist(pd.concat([df_h[~mask], v], ignore_index=True))
                        st.session_state.editing_exo.discard(exo_final); st.toast(f"Sauvé : {exo_base}", icon='💪'); st.rerun()
...
with tab3:
    if not df_h.empty:
        ...
        if not df_p.empty:
            ...
            p_cols = st.columns(3)
            meds, clss = ["🥇 OR", "🥈 ARGENT", "🥉 BRONZE"], ["podium-gold", "podium-silver", "podium-bronze"]
        ...
        if not df_rec.empty:
            ...
            one_rm_val = calc_1rm(best['Poids'], best['Reps'])
            c_res1, c_res2 = st.columns(2)
            c_res1.success(f"🏆 RECORD RÉEL\n\n**{best['Poids']}kg x {int(best['Reps'])}**")
            c_res2.info(f"⚡ 1RM ESTIMÉ\n\n**{one_rm_val:.1f} kg**")
            
            with st.expander("📊 Estimation Rep Max"):
                ests = get_rep_estimations(one_rm_val)
                cols_est = st.columns(len(ests))
                for idx, (r, p) in enumerate(ests.items()): cols_est[idx].metric(f"{r} Reps", f"{p}kg")
