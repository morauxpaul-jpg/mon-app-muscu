...
# Initialisation de la mémoire de l'app pour les modifications
if 'editing_exo' not in st.session_state:
    st.session_state.editing_exo = set()

# --- CSS : DESIGN & COULEURS TRANSLUCIDES ---
st.markdown("""
<style>
    .stApp {
...
def style_comparaison(row, hist_s1):
    # Sécurité contre le KeyError : on vérifie si l'historique existe et contient la colonne
    ...
    if not prev_set.empty:
        ...
        # RÈGLE ORANGE : Meilleur ratio mais poids plus léger
        if c_ratio > p_ratio and cw < pw:
            colors[1], colors[2] = o, o
        # RÈGLE VERT : Poids plus lourd (tout en vert)
        elif cw > pw:
            colors[1], colors[2] = v, v
        # RÈGLE ROUGE : Poids moins lourd (tout en rouge)
        elif cw < pw:
            colors[1], colors[2] = r, r
        # RÈGLE POIDS ÉGAL : Seules les reps changent
        elif cw == pw:
            if cr > pr: colors[1] = v
...
with tab2:
    if not prog: st.warning("Crée une séance d'abord !")
        ...
        choix_s = c_top1.selectbox("Séance :", list(prog.keys()))
        sem = c_top2.number_input("Semaine", min_value=1, value=1)
        
        # Bouton Skip Séance
        if st.button("🚫 Marquer SÉANCE ENTIÈRE comme loupée", use_container_width=True):
            skipped = [{"Semaine": sem, "Séance": choix_s, "Exercice": e, "Série": 1, "Reps": 0, "Poids": 0.0, "Remarque": "SÉANCE LOUPÉE ❌"} for e in prog[choix_s]]
            mask = (df_h["Semaine"] == sem) & (df_h["Séance"] == choix_s)
            save_hist(pd.concat([df_h[~mask], pd.DataFrame(skipped)], ignore_index=True))
            st.rerun()
        ...
        for exo in prog[choix_s]:
            with st.expander(f"🔹 {exo}", expanded=True):
                # 1. Historique (2 dernières fois)
                full_h = df_h[(df_h["Exercice"] == exo) & (df_h["Séance"] == choix_s) & (df_h["Semaine"] < sem)].sort_values(by="Semaine", ascending=False)
                last_weeks = full_h["Semaine"].unique()[:2]
                
                if len(last_weeks) > 0:
                    st.caption("🔍 Historique des 2 dernières séances :")
                    for w in last_weeks:
                ...
                h_s1 = full_h[full_h["Semaine"] == last_weeks[0]] if len(last_weeks) > 0 else pd.DataFrame(columns=["Série", "Reps", "Poids", "Remarque"])

                if not curr.empty and exo not in st.session_state.editing_exo:
                    st.caption(f"✅ Performance S{sem} :")
                    ...
                    if not curr.empty:
                        df_edit = pd.concat([curr[["Série", "Reps", "Poids", "Remarque"]], pd.DataFrame({"Série": [int(curr["Série"].max()+1)], "Reps": [0], "Poids": [0.0], "Remarque": [""]})], ignore_index=True)
                    else:
                        df_edit = pd.DataFrame({"Série": [1, 2, 3], "Reps": [0,0,0], "Poids": [0.0,0.0,0.0], "Remarque": ["","",""]})

                    ed = st.data_editor(df_edit, num_rows="dynamic", key=f"e_{exo}", use_container_width=True, column_config={"Poids": st.column_config.NumberColumn(format="%g")})
                    ...
                    if c_sk.button(f"🚫 Skip l'exo", key=f"sk_{exo}"):
                        sk = pd.DataFrame([{"Semaine": sem, "Séance": choix_s, "Exercice": exo, "Série": 1, "Reps": 0, "Poids": 0.0, "Remarque": "EXO SKIP 🚫"}])
                        mask = (df_h["Semaine"] == sem) & (df_h["Séance"] == choix_s) & (df_h["Exercice"] == exo)
                        save_hist(pd.concat([df_h[~mask], sk], ignore_index=True)); st.rerun()
...
with tab3:
    if not df_h.empty:
        ...
        col2.metric("Séances", len(df_h[df_h["Poids"] > 0].groupby(["Semaine", "Séance"])))
        ...
        if not df_e.empty:
            ...
            if not df_v.empty:
                ...
                st.success(f"🏆 Record : **{max_s['Poids']} kg x {int(max_s['Reps'])}**")
