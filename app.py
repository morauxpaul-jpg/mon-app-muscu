...
def style_comparaison(row, hist_prev):
    """Logique de couleur Vert/Rouge comparée à la séance précédente."""
...
with tab1:
    ...
    for idx_j, j in enumerate(jours):
        with st.expander(f"⚙️ {j}"):
            # OPTION REORGANISER / SUPPRIMER SEANCE
...
with tab2:
    if not prog: st.warning("Crée une séance dans le programme.")
        ...
        # OPTION SEANCE LOUPÉE
        if st.button("🚫 Séance Loupée ❌ (Marquer tout loupé)", use_container_width=True):
            sk_rows = [{"Cycle": cycle_act, "Semaine": sem_stk, "Séance": choix_s, "Exercice": e["name"], "Série": 1, "Reps": 0, "Poids": 0.0, "Remarque": "Séance Loupée ❌"} for e in prog[choix_s]]
        ...
        for i, ex_obj in enumerate(prog[choix_s]):
            ...
            with st.expander(f"Saisie : {exo}", expanded=True):
                ...
                if not last_s.empty:
                    st.caption("🔍 Historique (Max 2 dernières séances) :")
                    for _, r_s in last_s.iterrows():
                        cp, sp = r_s["Cycle"], r_s["Semaine"]
                        st.write(f"**C{cp} - S{sp if sp != 0 else 10}**")
