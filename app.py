...
# --- 2. CSS : DESIGN NÉON & LOOK MODERNE ---
st.markdown("""
<style>
    .stApp {
    ...
    /* EFFET NÉON SUR LES CHIFFRES CLÉS */
...
# --- CONNEXION & GESTION GOOGLE SHEETS ---
@st.cache_resource
...
with tab1:
    ...
    for idx_j, j in enumerate(jours):
        with st.expander(f"⚙️ {j}"):
            ...
            if c2.button("🗑️ Supprimer Séance", key=f"del_s_{j}"):
...
# --- ONGLET 2 : MA SÉANCE ---
with tab2:
    if not prog: st.warning("⚠️ Aucune séance trouvée. Crée-en une dans l'onglet 'Programme'.")
    else:
        ...
        for i, exo in enumerate(prog[choix_s]):
            ...
            with st.expander(f"Détails : {exo}", expanded=True):
                # FIX NAMEERROR ICI : Correction de full_h en full_exo_h
                ...
                if len(last_w) > 0:
                    st.caption("🔍 Historique (Dernières fois) :")
                    for w in last_w:
