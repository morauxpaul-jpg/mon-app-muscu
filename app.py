...
with tab1:
    ...
    for idx_j, j in enumerate(jours):
        with st.expander(f"⚙️ {j}"):
            ...
            ns = c_add2.number_input("Sets", 1, 15, 3, key=f"ns_{j}")
    ...
    if prog:
        ...
        for i, ex_obj in enumerate(prog[choix_s]):
            ...
            with st.expander(f"Saisie : {exo}", expanded=True):
                ...
                cond_hist = (f_h["Cycle"] < c_act) | ((f_h["Cycle"] == c_act) & (f_h["Semaine"] < s_act))
...
# --- TAB 3 : PROGRÈS (PODIUM & FIX BODYWEIGHT) ---
with tab3:
    if not df_h.empty:
        ...
        # FIX : Accepte Reps > 0 même si Poids = 0
        df_rec = df_e[(df_e["Poids"] > 0) | (df_e["Reps"] > 0)].copy()
        
        if not df_rec.empty:
            best = df_rec.sort_values(["Poids", "Reps"], ascending=False).iloc[0]
            st.success(f"Record : **{best['Poids']} kg x {int(best['Reps'])}** (1RM théorique: {calc_1rm(best['best']['Poids'], best['Reps']):.1f} kg)" if 'best' in locals() else f"Record : **{best['Poids']} kg x {int(best['Reps'])}**")
            st.line_chart(df_rec.groupby(["Cycle", "Semaine"])["Poids"].max())
